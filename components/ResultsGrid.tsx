...
function DestCard({ dest, index }: { dest: Destination, index: number }) {
  const total = dest.vol + dest.hotel
  const meteoIcon = dest.meteo ? (dest.meteo.soleil >= 6 ? '☀️' : dest.meteo.soleil >= 3 ? '⛅' : '☁️') : '?'
  ...
  return (
    <div className={`bg-slate-900 rounded-2xl p-5 border ${index === 0 ? 'border-sky-500' : 'border-slate-800'} hover:border-slate-600 transition-colors`}>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        ...
        <span className="text-xs bg-sky-500/10 text-sky-400 px-2 py-1 rounded-lg">
          ✈ ~{dest.vol}€ A/R
          <span className="opacity-60 ml-1">{dest.volSource === 'reel' ? '● réel' : '● estimé'}</span>
        </span>
        <span className="text-xs bg-amber-500/10 text-amber-400 px-2 py-1 rounded-lg">
          🏨 ~{dest.hotel}€/nuit · Total ~{total}€
        </span>
      </div>
      ...
      <div className="flex gap-2">
        <a href={dest.kiwiUrl} target="_blank" rel="noopener noreferrer"
