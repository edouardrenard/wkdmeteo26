...
function buildPopup(dest: Destination): string {
  ...
  const tRow = (t: typeof t0) => t ? `
    <div style="display:flex;justify-content:space-between;margin-bottom:4px;font-size:12px">
      <span style="color:#94A3B8">${tIcon(t.type)} ${tLabel(t.type)} A/R${t.duree ? ` (${t.duree})` : ''}</span>
  ...
  return `
    <div style="font-family:system-ui;min-width:240px;color:white">
      ...
      <div style="background:#1E293B;border-radius:8px;padding:8px;margin-bottom:8px">
        ...
        ${tRow(t2)}
        <div style="display:flex;justify-content:space-between;margin-bottom:4px;font-size:12px">
          <span style="color:#94A3B8">🏨 Hôtel (~${dest.hotelNuit}€/nuit)</span>
        ...
        <div style="border-top:1px solid #334155;padding-top:6px;margin-top:4px;display:flex;justify-content:space-between;font-size:13px">
          ...
          <span style="font-weight:700;color:${tc}">~${dest.totalEstime}€</span>
        </div>
      </div>
    ...
    const update = async () => {
      ...
      results.forEach(dest => {
        ...
        const labelIcon = L.divIcon({
          className: '',
          html: `<div style="background:${color};color:white;padding:2px 8px;border-radius:10px;font-size:11px;font-weight:700;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,0.5);border:1px solid rgba(255,255,255,0.25)">${tIcon(dest.meilleurTransport.type)} ${dest.nom} ${dest.scoreGlobal}</div>`,
          iconAnchor: [0, 0],
        })
